...
class drive_handler():
    ...
    def write_blob_to_slack(self, blob, blob_id):
        ...
        if not(1<=int(blob_id)<=255):
            print("Invalid ID: must be in [1, 255]: "+blob_id)
...
class file_handler():
    ...
    def make_blob(self):
        ...
        blob_name = 'blob-input-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + '.tar.gz'
    ...
    def extract_files_from_blob(self, blob, save_path):
        ...
        blob_path = 'staging/' + 'blob-output-' + datetime.now().strftime("%Y-%m-%d-%H-%M-%S") + '.tar.gz'
        blob_file = open(save_path, 'wb')
